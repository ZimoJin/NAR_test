...
function renderOverlapTable(results, inserts, naConc, mgConc, primerConc) {
  ...
  // Use the exact overlap sequences that are already stored on the primer sets.
  // This guarantees the overlap table matches the bold-highlighted overlaps in the primer table.
  const primerSets = (results && Array.isArray(results.primers)) ? results.primers : [];
  const rows = [];
  primerSets.forEach((p, idx) => {
    const insertLabel = (p && p.insertName) || (inserts && inserts[idx] && inserts[idx].label) || `Insert #${idx + 1}`;
