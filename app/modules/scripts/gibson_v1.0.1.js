      ...
      <div class="insert-body">
        ...
        <div style="display: flex; flex-direction: column; gap: 6px; margin-top: 6px;">
          <div style="display: flex; gap: 4px; align-items: center;">
            <input type="text" class="insert-linker" placeholder="Linker (optional)" list="linker-suggestions-${insertCount - 1}" style="flex: 1; font-family: monospace; font-size: 0.85rem;" autocomplete="off">
            <datalist id="linker-suggestions-${insertCount - 1}">
              ...
              <option value="EAAAKKEAAAKK">2x(EAAAK)</option>
              <option value="EAAAKKEAAAKKEAAAKK">3x(EAAAK)</option>
              <option value="HHHHHH">His-tag (6xHis)</option>
              <option value="HHHHHHHH">His-tag (8xHis)</option>
              <option value="DYKDDDDK">FLAG-tag</option>
              <option value="YPYDVPDYA">HA-tag</option>
              <option value="EQKLISEEDL">c-Myc-tag</option>
              <option value="GST">GST-tag</option>
              <option value="MKLLVVVFCLGIAP">Strep-tag II</option>
            </datalist>
...
function designGibsonAssembly(vector, inserts, linkers = [], opts) {
  ...
  if (mode === 'enzyme') {
    ...
    if (original0InFragment) {
      ...
      // We need to rotate so that the position corresponding to original 0 becomes the new 0
      // Since assembledCircle starts with backboneSeq, we rotate by -offsetInFragment
      ...
      // Rotate so that this position becomes the new 0
  ...
  if (insertPositions.length > 0 && insertPositions[0].start > 0) {
    leftOverlap = assembledSeq.slice(Math.max(0, insertPositions[0].start - overlapLen), insertPositions[0].start);
  }
  if (insertPositions.length > 0) {
    rightOverlap = assembledSeq.slice(insertPositions[insertPositions.length - 1].end, 
                          Math.min(assembledSeq.length, insertPositions[insertPositions.length - 1].end + overlapLen));
  }
  ...
  for (let i = 0; i < insertPositions.length - 1; i++) {
    ...
    if (currentLinkerLen >= overlapLen) {
      ...
      const overlapStartInLinker = Math.floor((currentLinkerLen - overlapLen) / 2);
      const overlapEndInLinker = overlapStartInLinker + overlapLen;
      const junctionOverlap = assembledSeq.slice(
        linkerStartInAssembled + overlapStartInLinker,
        linkerStartInAssembled + overlapEndInLinker
      );
      ...
      const linkerMidInAssembled = currentLinkerLen > 0
        ...
        : linkerStartInAssembled;
      const halfOverlap = Math.floor(overlapLen / 2);
      const backPart = assembledSeq.slice(linkerMidInAssembled - halfOverlap, linkerMidInAssembled);
      const forwardPart = assembledSeq.slice(linkerMidInAssembled, linkerMidInAssembled + halfOverlap);
...
function renderResults(results, vector, inserts) {
  ...
  `;
  resultsContent.appendChild(asmDiagramCell);
  ...
  `;
  resultsContent.appendChild(overlapTableCell);
...
function renderOverlapTable(results, inserts, naConc, mgConc, primerConc) {
  ...
  const PAL = ['#3b82f6', '#ec4899', '#10b981', '#f59e0b', '#8b5cf6', '#ef4444'];
  ...
  // Left overlap
  if (results.leftOverlap) {
    const seq = results.leftOverlap;
    const len = seq.length;
  ...
  // Junction overlaps
  results.junctionOverlaps.forEach((ov, idx) => {
    const seq = ov;
    const len = seq.length;
    const gc = Core.gcPct(seq);
    const tm = Core.tmcalNN(seq, naConc, mgConc, primerConc);
  ...
  // Right overlap
  if (results.rightOverlap) {
    const seq = results.rightOverlap;
    const len = seq.length;
    const gc = Core.gcPct(seq);
    const tm = Core.tmcalNN(seq, naConc, mgConc, primerConc);
    const color = PAL[colorIdx % PAL.length];
...
function updateAssemblyLegend(results, vector, inserts) {
  ...
  let legend = '';
  legend += `<div><strong>L1</strong> DNA Ladder</div>`;
...
function initGibsonModule() {
  ...
  attachHelpToInput('primer-conc', "Primer concentration used for Tm calculation. Typical range: 200–1000 nM.", 'Help: Primer concentration');
  attachHelpToInput('na-conc', "Monovalent cation concentration (Na<sup>+</sup>, mM) for salt-corrected Tm calculation. Typical range: 0–200 mM.", 'Help: Na+ concentration');
  attachHelpToInput('mg-conc', "Divalent cation concentration (Mg<sup>2+</sup>, mM) for salt-corrected Tm calculation. Typical range: 0–5 mM. Set to 0 if unknown.", 'Help: Mg2+ concentration');
