...
function subseqCircular(seq, start, len){
  ...
  function syncVectorHeight(){
    ...
    const insertsDiv=document.getElementById('inserts-container');
    if(!leftTA || !insertsDiv) return;
    const rect = insertsDiv.getBoundingClientRect();
    const target = Math.max(140, Math.min(1200, rect.height || insertsDiv.scrollHeight || 140));
  ...
  function setupInsertRowListeners(row) {
    ...
    if (insertDemoBtn) {
      insertDemoBtn.addEventListener('click', async () => {
        ...
        try {
          const base = `${window.location.origin}/app/modules/contents/demo/`;
  ...
  document.getElementById('gg-run').addEventListener('click', async ()=>{
    try{
      ...
      const insSites1 = findRESites(insClean, enz1.site);
      const insSites2 = findRESites(insClean, enz2.site);
      const insWarns = [];
      if(insSites1.length) insWarns.push(enz1Name);
      if(enz2Name !== enz1Name && insSites2.length) insWarns.push(enz2Name);
      if(insWarns.length){
        insertSiteWarnHtml =
          `<div class="aside" style="margin-top:6px;color:#b91c1c">
            Warning: selected enzymes (${insWarns.join(', ')}) have recognition sites in the insert.
      ...
      const blunt1 = !enz1.sticky;
      const blunt2 = !enz2.sticky;
      const sameSticky = (enz1 && enz2 && enz1.sticky && enz2.sticky && (enz1.sticky === enz2.sticky));


      // 1) Single-enzyme combinations are ack if the longest fragment behaves like a single-enzyme backbone.
      if(n1 > 1 || n2 > 1){
        if(digestInfo.pseudoSingle){
          const reasonShort = `pseudo single digest (longest fragment flanked by same enzyme)`;
          const warningMsg =
...
if(n1 === 0 || n2 === 0){
        const reasonShort = (n1===0 && n2===0)
          ? `no ${enz1Name} / ${enz2Name} sites in vector`
          : `missing ${ (n1===0 ? enz1Name : enz2Name) } site in vector`;
        const warningMsg = (n1===0 && n2===0)
          ? `There are no ${enz1Name} / ${enz2Name} sites in the vector. Please select appropriate enzymes or vector.`
          : `The vector lacks a ${ (n1===0 ? enz1Name : enz2Name) } site. Please select other enzymes or modify the vector.`;

        if(primerNode){
          primerNode.innerHTML = `<div class="aside">N/A (${reasonShort})</div>`;
        }
        if(qcNode){
          qcNode.innerHTML = `<div class="aside">N/A (${reasonShort})</div>`;
        }
        ...
        if(n1===0 && n2===0){
          ...
          showGel(vectorLen, [vectorLen], [], 0, null, insertNames, vectorName, enz1Name+'/'+enz2Name, '');
          GGX_STATE.scIdx = new Set([1]);
          ggxDrawGel();
          ggxUpdateLegend();
        }
      ...
      const design = designREPrimers(insertRaw, enz1Name, enz2Name, clampN, tmTarget, Na, conc);
      ...
      const stickyF = primers[0] && primers[0].OH_L ? primers[0].OH_L : (enz1.sticky||'');
      const stickyR = primers[0] && primers[0].OH_R ? primers[0].OH_R : (enz2.sticky||'');
      ...
      showGel(vectorLen, digestInfo.allFrags || [digestInfo.longestFragLen], [pcrSize], assembledLen, assembledDigestFrags, insertNames, vectorName, enz1Name+'/'+enz2Name, asmName);
...
function exportResultsPdf(){
  try{
    const node = document.getElementById('results-wrap');
    if(!node){
      if(window._ggShowModal){
        window._ggShowModal('No results to export.');
      }else{
    ...
    if(typeof html2canvas === 'undefined' || typeof window.jspdf === 'undefined'){
      if(window._ggShowModal){
        window._ggShowModal('PDF libraries failed to load. Please check your internet connection.');
      }else{
    ...
    html2canvas(node, {scale: 2, useCORS: true, backgroundColor: '#ffffff'}).then(canvas=>{
      ...
      pdf.save('RE_cloning_results(' + safeAsm + ').pdf');
      window.scrollTo(origScrollX, origScrollY);
    }).catch(err=>{
      console.error(err);
      if(window._ggShowModal){
        window._ggShowModal('Failed to generate PDF: ' + err.message);
      }else{
    ...
    console.error(e);
    if(window._ggShowModal){
      window._ggShowModal('Failed to generate PDF: ' + e.message);
    }else{
...
(function(){
  const overlay = document.getElementById('gg-modal');
  const textEl = document.getElementById('gg-modal-text');
  const okBtn = document.getElementById('gg-modal-ok');
...
document.getElementById('gg-download-btn').addEventListener('click',()=>{
  try{
    ...
    const modeEl = document.getElementById('gg-download-type');
    ...
    if(mode === 'primers'){
      const node = document.getElementById('primer-table');
