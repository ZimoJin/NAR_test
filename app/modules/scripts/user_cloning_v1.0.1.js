...
function renderPrimerSets(results, naConc, mgConc, primerConc) {
  ...
  // Vector primers
  const vecFwdAnalysis = analyzePrimer('Vector-F', results.vector.F.seq, results.vector.F.coreSeq, naConc, mgConc, primerConc);
  const vecRevAnalysis = analyzePrimer('Vector-R', results.vector.R.seq, results.vector.R.coreSeq, naConc, mgConc, primerConc);
  ...
  const fLabel = `Insert${firstIndex}-F`;
  const rLabel = `Insert${firstIndex}-R`;
...
function initUserDemoSetButton() {
  ...
  demoSetBtn.addEventListener('click', async () => {
    try {
      ...
      if (pcrReverse) pcrReverse.value = 'ATCCGGGGTUTTTTCTCCTTGACGTTAAAGTATAGAGG';
    } catch (e) {
...
function initResetButton() {
  const resetBtn = $('global-reset') || $('clear-btn');
  if (!resetBtn) return;
...
function renderGel(results, vector, inserts) {
  ...
  const scEffectiveSize = Math.floor(vector.seq.length * 0.85);
  lanes.push([scEffectiveSize]); // L2: Uncut vector (SC)
  ...
  // Draw gel
  const highlightIndices = new Set([1, 2, lanes.length - 1]);
  VIZ.drawGel('gg-gel-canvas', lanes, lad.sizesKb.map(k => k * 1000), lad.boldKb.map(k => k * 1000), lad.name, { highlightIndices });
  
  // Update gel legend
  const gelLegend = $('ggx-legend');
  if (gelLegend) {
    let legend = '';
    legend += `<div><strong>L1</strong> DNA Ladder</div>`;
    legend += `<div><strong>L2</strong> Uncut vector (${vecName}, SC): ${vector.seq.length} bp</div>`;
    legend += `<div><strong>L3</strong> PCR linearized vector: ${vector.seq.length} bp</div>`;
    if (insertCount > 0) {
      for (let i = 0; i < insertCount; i++) {
        const laneIdx = 4 + i;
        const name = insertNames[i] || `Insert${i + 1}`;
        const len = inserts[i]?.length || 0;
        legend += `<div><strong>L${laneIdx}</strong> PCR of insert (${name}): ${len} bp</div>`;
      }
    }
    const assembledLen2 = results.assembledSeq
      ? results.assembledSeq.length
      : (window.currentAssembledSeq ? Core.normalizeSeq(window.currentAssembledSeq).length : 0);
    const assembledLaneIdx2 = 4 + insertCount;
    legend += `<div><strong>L${assembledLaneIdx2}</strong> Assembled plasmid (${assembledName}): ${assembledLen2} bp</div>`;
    gelLegend.innerHTML = legend;
  }
}
