...
export function designOePcrPrimers(mutatedTemplate, codonStart, opts = {}) {
  ...
  // Calculate overlap Tms (for Fmut and Rmut only, F1 and R2 don't have overlap)
  const FmutOverlapTm = tmSaltCorrected(FmutOverlapTail, conc_nM, na_mM, mg_mM);
  const RmutOverlapTm = tmSaltCorrected(RmutOverlapTail, conc_nM, na_mM, mg_mM);
...
function downloadPrimersFASTA() {
  ...
  for (const r of primersToDownload) {
    // Generate operation string for naming
    let operationStr = r.operationStr || r.description || `edit${r.id || r.editIdx + 1}`;

    // Generate primer names
    const baseName = currentSeqName || 'fragment';
    const f1Name = `${baseName}-F`;
    const r2Name = `${baseName}-R`;
    const fmutName = `${baseName}(${operationStr})-F`;
    const rmutName = `${baseName}(${operationStr})-R`;
    ...
    for (const p of primers) {
      const primerInfo = r[p.key];
      if (primerInfo && primerInfo.seq) {
        fastaContent += `>${p.name}\n`;
        fastaContent += `${primerInfo.seq}\n`;
      }
    }
  }
