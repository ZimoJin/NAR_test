...
function initQC_V4(container) {
  ...
  if (demoSetBtn) {
    demoSetBtn.onclick = function(){
      ...
      const rev = container.querySelector("#rev");
      if (fwd) {
        fwd.value = ">primer1-F\nATGGTGAGRAAGGGCGAGGAG\n>primer2-F\nGCTACCGTAACTGCGTAGCAA";
      }
      if (rev) {
        rev.value = ">primer1-R\nCTTGTACAGCTCGTCCATGCC\n>primer2-R\nTTGCTACGCAGTTACGGTAGC";
      }
    };
  }
...
// Auto-initialize immediately (for embedded mode)
// The script runs after HTML is injected, so we can initialize right away
(function() {
  const container = document.getElementById('module-content') || document.body;
  const demoBtn = container.querySelector("#demo");
  if (demoBtn) {
