...
import * as CORE from './modules/scripts/core_v1.0.1.js';
import * as VIZ from './modules/scripts/bio_visuals_v1.0.1.js';

// Expose Core and VIZ to global scope for inline scripts
window.CORE = CORE;
window.VIZ = VIZ;
...
const rc = CORE.reverseComplementSeq;
const revcompSeq = CORE.reverseComplementSeq;
const gcPct = CORE.gcPct;
...
// Legacy function aliases (may not exist in core.js, provide fallbacks)
...
// Expose fallbacked helpers for inline HTML scripts (Golden Gate)
window.pickCorePrimerForward = pickCorePrimerForward;
window.pickCorePrimerReverse = pickCorePrimerReverse;
window.makeSmartClamp = makeSmartClamp;
    ...
    async function loadModule(moduleName) {
        ...
        if (standaloneModules.includes(moduleName)) {
            ...
            try {
                ...
                // Run init functions after DOM is ready
                setTimeout(() => {
                    switch (moduleName) {
                        case 'golden-gate': {
                            setTimeout(() => {
