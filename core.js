 ...
 *  - keeping only valid IUPAC DNA symbols (A,C,G,T,U,R,Y,S,W,K,M,B,D,H,V,N)
 * U is mapped to T at the end (for convenience).
 */
export function normalizeSeq(raw = "") {
  if (!raw) return "";
  const up = raw.toUpperCase().replace(/[^ACGTURYSWKMBDHVN]/g, "");
  // Map U to T
...
/**
 * Derive vector backbone cut positions and overhangs for a Type IIS enzyme
 * on a circular plasmid, using the same logic as the legacy v3.3 Golden Gate
 * implementation (longest fragment between adjacent cuts is treated as backbone).
 *
 * Returns an object:
 *   { leftOH, rightOH, cutL, cutR }
 * or null if sites/cuts are insufficient.
 */
export function deriveTypeIISVectorOverhangs(seq, enzName) {
  const s = normalizeSeq(seq);
  const N = s.length;
  if (!N) return null;

  const enz = getEnzyme(enzName);
  if (!enz || enz.class !== "typeIIS") return null;

  const cutsRaw = computeCutsTypeIIS(s, enzName);
  if (!cutsRaw.length) return null;

  const cuts = cutsRaw.map(c => {
    const pos = ((c.cutPos % N) + N) % N;
    return pos;
  });
  ...
  function subseqCircularLocal(start, len) {
    if (len <= 0) return "";
    start = ((start % N) + N) % N;
    if (start + len <= N) return s.slice(start, start + len);
    return s.slice(start) + s.slice(0, (start + len) % N);
  }

  const ohLen = enz.overhang || 4;
  const leftOH  = reverseComplementSeq(subseqCircularLocal(cutR, ohLen));
  const rightOH = subseqCircularLocal(cutL, ohLen);

  return { leftOH, rightOH, cutL, cutR };
}

// ============================================================================
// 10. Higher-level restrictionn / overhang / ligation helpers
// ============================================================================
...
export function findAmplicons(templateSeq, fwdSeq, revSeq) {
  ...
  if (fwd.length) {
    for (let i = 0; i <= tpl.length - fwd.length; i++) {
      if (tpl.slice(i, i + fwd.length) === fwd) hitsF.push(i);
    }
  }

  // Find all reverse-primer matches (via reverse complement)
  if (revRC.length) {
    for (let i = 0; i <= tpl.length - revRC.length; i++) {
      if (tpl.slice(i, i + revRC.length) === revRC) hitsR.push(i);
    }
  }
...
// ============================================================================
// 12. Sequence Generation Utilities (Restored and Enhanced from V3.3)
// ============================================================================

/**
 * Generates a random DNA spacer of length n.
 * Used to separate the enzyme site from the overhang to prevent secondary structures.
 */
export function makeRandomSpacer(n = 1) {
  if (n <= 0) return "";
  const bases = ['A', 'C', 'G', 'T'];
  let s = "";
  for (let i = 0; i < n; i++) {
...
/**
 * Generates a "Smart" 5' Clamp (Protective Bases) using thermodynamic QC.
 * Enhancements over v3.3:
 * 1. GC Content control (keeps it balanced, 30-70%).
 * 2. Thermodynamic checks: Uses hairpinScan and selfDimerScan.
 * 3. Homopolymer check.
 */
export function makeSmartClamp(length = 5) {
  ...
  const randomSeq = (n) => {
    let s = "";
    for (let i = 0; i < n; i++) s += BASES[Math.floor(Math.random() * 4)];
    return s;
  };
  
  for (let i = 0; i < MAX_ATTEMPTS; i++) {
    const seq = randomSeq(length);

    // 1. Homopolymer check
    if (hasHomopolymer(seq, MAX_HOMO + 1)) continue;

    // 2. GC Content check
    const gc = gcPct(seq);
    if (gc < MIN_GC || gc > MAX_GC) continue;

    // 3. Thermodynamic checks (only if length allows folding)
    if (length >= 4) {
      const hp = hairpinScan(seq);
      if (hp && hp.dG < -1.0) continue; // Avoid strong hairpins
      
      const sd = selfDimerScan(seq);
      if (sd && sd.dG < -3.0) continue; // Avoid strong dimers
    }

    return seq;
  }

  // Fallback: neutral pattern
  return "GCATGCATGCATGCATGCAT".slice(0, length);
}
